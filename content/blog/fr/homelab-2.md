...
Cela signifie également que lorsque quelque chose casse — et quelque chose casse toujours — vous pouvez reconstruire un nœud dans le temps qu'il faut à l'installateur pour s'exécuter. Pas d'étapes manuelles à retenir, pas de documentation à consulter. Démarrez, attendez, terminé.
...
Ubuntu Server 24.04 est livré avec un installateur intégré appelé **autoinstall**. Il lit un fichier de configuration appelé `user-data` et exécute l'installation entièrement sans intervention. Pas de clics, pas de messages, pas d'interaction au clavier.
...
Une configuration, trois machines. C'est pourquoi le README mentionne cela comme une décision de conception délibérée : des noms d'interface cohérents sont ce qui rend l'approche d'un seul fichier viable.
...
Cette incohérence m'a agacé plus que probablement elle n'aurait dû. Utiliser `ih-node-01` dans l'inventaire Ansible, les configurations SSH et kubeconfig tout en devant utiliser `ih-node-1` pour la résolution DNS réelle est le genre de chose qui provoque des échecs mystérieux à 1h du matin.
...
La solution la plus simple : renommez simplement les nœuds en `ih-node-1`, `ih-node-2`, `ih-node-3`. Pas de zéros non significatifs. Maintenant, le nom d'hôte sur la machine correspond à ce que le routeur résout. Cohérence rétablie.
...
L'utilisateur est créé par le bloc `identity` de autoinstall :
...
## 10 - Garder l'ordinateur portable actif lorsqu'il est fermé
...
Sans cela, fermer le capot pour ranger la machine la mettrait en veille et le cluster s'effondrerait. J'ai appris cela à mes dépens.

---

## 11 - Leçons apprises

Parcourir ce processus sur trois machines différentes m'a appris quelques choses qui valent la peine d'être écrites.

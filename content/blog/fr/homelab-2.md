---
title: Homelab - 2 - Du Bare Metal à un Nœud de Cluster Démarrable
...
*Cet article couvre exactement cela — la couche système d'exploitation. Comment je suis passé de trois machines vierges à trois nœuds Ubuntu identiques et accessibles via le réseau, provisionnés automatiquement à partir d'une seule clé USB et d'un seul fichier de configuration.*
...
Mais en approfondissant mes connaissances sur Kubernetes au travail et en lisant sur le fonctionnement réel des choses en coulisses, j'ai commencé à remarquer à quel point K3s abstrait. Tout est dans un seul binaire. La base de données embarquée, l'équilibreur de charge de service intégré, la façon dont il regroupe containerd et le plugin CNI — tout cela est vraiment pratique, mais cela signifie que vous n'apprenez pas vraiment les primitives brutes de Kubernetes.
...
La première fois que j'ai démarré depuis la clé Ventoy sur l'un des ordinateurs portables, j'ai obtenu cette erreur :
...
La seule différence est que j'utilise un SSD directement au lieu d'une carte SD, ce qui est plus adapté à un cluster Kubernetes. Le résultat est équivalent — un nœud sans tête et accessible via le réseau sans configuration manuelle.
...
Il existe une approche alternative où vous servez `user-data` via HTTP et passez un paramètre noyau au démarrage :
...
Les nœuds n'ont pas de connexion filaire au routeur. Il n'y a pas de switch réseau dans ma configuration. Tout fonctionne via le WiFi. C'est bien pour un homelab, mais cela a créé deux problèmes distincts pendant le provisionnement.
...
Ubuntu utilise par défaut des noms d'interface réseau "prévisibles" comme `enp3s0` pour Ethernet ou `wlp13s0` pour WiFi. La partie "prévisible" signifie que le nom encode la topologie du bus PCI du périphérique.
...
La solution consiste à désactiver complètement la dénomination prévisible en passant des paramètres noyau au démarrage :

```
net.ifnames=0 biosdevname=0
```

Avec ces drapeaux, le noyau revient aux anciens noms : `eth0` pour la première interface Ethernet, `wlan0` pour la première interface WiFi. Ceux-ci sont cohérents sur les trois machines, quelles que soient les différences matérielles.
...
*Je ne peux pas contrôler cela directement sur mon routeur car c'est un routeur grand public... Big Up à mon fournisseur FAI*

Cette incohérence m'a agacé plus que ça n'aurait dû. Utiliser `ih-node-01` dans l'inventaire Ansible, les configurations SSH et kubeconfig mais devoir utiliser `ih-node-1` pour la résolution DNS réelle est le genre de chose qui provoque des échecs mystérieux à 1h du matin.
...
Ceci produit un hash salé SHA-512 que cloud-init accepte. Le mot de passe réel n'a pas beaucoup d'importance car la connexion par mot de passe via SSH est désactivée de toute façon :
...
Une autre chose qui mérite d'être mentionnée : les trois machines sont des ordinateurs portables (l'une est un raspberrypi). Elles fonctionnent sans écran. J'ai ajouté la configuration suivante après la première installation pour m'assurer qu'elles ne se suspendent pas lorsque je ferme le capot :

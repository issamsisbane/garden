...
J'ai donc construit mon propre laboratoire à la maison, j'ai décidé de l'appeler mon HomeLab :) (Attendez, c'est déjà une chose ? Je ne l'ai pas inventé ?)
...
J'ai préféré le **bare metal** aux VM ou aux instances cloud pour acquérir une expérience pratique de la gestion de la pile complète.
...
La meilleure pratique en production est de séparer le plan de contrôle des charges de travail, mais je conserve tous les nœuds à double usage pour maintenir le quorum sans ajouter de matériel supplémentaire.
...
C'est un autre vieil ordinateur portable que j'ai réaffecté. J'ai simplement installé Ubuntu Server dessus. Pas d'histoire dramatique, juste un autre nœud pour mon cluster.
...
C'est utile car si nous venons de l'exporter, nous perdrions les variables en fermant le shell. Je ne sais pas si j'en aurai besoin plus tôt. Bien sûr, je ne le commets pas dans git.
...
Cette commande permet de configurer dans quel référentiel et quelle branche nous voulons que flux surveille. Ensuite, Flux créera le fichier manifeste dans le référentiel pour se gérer lui-même.
...
Je peux enfin me qualifier de véritable ingénieur car j'ai un laboratoire à domicile où je peux expérimenter librement !
...
J'ai maintenant un objectif qui est d'avoir une pièce entière dédiée à mon homelab, un véritable centre de données !

...
J'ai passé les **trois dernières années** à travailler avec des clusters Kubernetes de niveau production au travail. Bien que cette expérience m'ait exposé à des problèmes réels, elle laissait peu de place à l'expérimentation ou aux plongées approfondies dans des sujets qui me passionnent. Pour combler ce vide, je lance un homelab personnel où je peux casser des choses sans conséquence et explorer de nouvelles technologies à ma guise.
...
J'ai préféré le **bare metal** aux machines virtuelles ou aux instances cloud pour acquérir une expérience pratique de la gestion de la pile complète.
...
- Créer des alertes et des tableaux de bord de surveillance pour tout surveiller
...
Au travail, je n'utilise actuellement qu'Argo, alors je voulais explorer ce que Flux a à offrir.
...
Pour modifier ce comportement et utiliser etcd pour notre cluster HA à 3 nœuds, j'ai dû mettre à jour le service à `/etc/systemd/system/k3s.service` pour ajouter `cluster-init` :
...
Nous devons créer un jeton d'accès personnel GitHub avec des `permissions de référentiel`.
...
J'ai maintenant un objectif qui est d'avoir une pièce entière dédiée à mon homelab, un vrai datacenter !

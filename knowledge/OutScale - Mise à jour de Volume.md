...
> [!IMPORTANT] Prérequis à vérifier
> Il faut bien penser à ajouter son ip dans API Access du site Outscale qui peut avoir changer et utiliser la nouvelle api OAPI.
...
> Il est plus simple d'utiliser l'api que de se battre avec la cli osc
> J'ai créer un script pour faire ça plus facilement
> Ou alors utiliser la nouvelle cli oapi
...
oapi-cli --profile "default" ReadVolumes \
    --Filters '{
        "VolumeIds": ["vol-a04ac06c"]
      }'

oapi-cli --profile "default" UpdateVolume \
  --VolumeId "vol-a04ac06c" \
  --Size 45
...
### 4. Configuration du volume sur Windows uniquement
On accède à la vm via RDP et on accède à diskmgmt.msc depuis le menu démarrer. On doubleclick sur notre volume puis etendre volume et on suit les étapes jusqu'a la fin. 
Notre volume est bien configuré !
